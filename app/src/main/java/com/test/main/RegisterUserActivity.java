...
public class RegisterUserActivity extends Activity{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		back= (TextView) findViewById(R.id.ac_re_user_no_re);
	...
	class RegisterButtonClickListener implements OnClickListener
	{
		...
		public void onClick(View v) {
			...
			//Bmob register
			if (!Util.isNetworkConnected(v.getContext())) {
				toast("亲, 木有网络 ( ⊙ o ⊙ ) ");
			} else if (user_text.equals("")|| password_text.equals("") || !password_text.equals(re_pass_text)) {
				toast("信息不完整或确认密码不正确，请重新填写。");
			} else
			{
				user = new Shop();
